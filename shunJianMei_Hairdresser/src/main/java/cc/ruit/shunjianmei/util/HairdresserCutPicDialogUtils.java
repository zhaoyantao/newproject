...
private void startUserPhotoZoom(Uri uri){
	...
	// outputX outputY 是裁剪图片宽高
	intent.putExtra("outputX", ScreenUtils.dip2px(activity, 100));
	intent.putExtra("outputY", ScreenUtils.dip2px(activity, 100));
	intent.putExtra("return-data", true);
	if (fragment != null) {
		fragment.startActivityForResult(intent, PHOTORESOULT);
	} else {
...
//	}
	...
	private String saveBitmap(String bitName, Bitmap mBitmap) {
		...
		try {
			...
			mBitmap.compress(Bitmap.CompressFormat.PNG, 60, fOut);//48186
