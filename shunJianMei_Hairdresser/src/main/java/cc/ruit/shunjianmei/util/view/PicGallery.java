...
public class PicGallery extends Gallery {
	...
	public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX,
		...
		if (view instanceof MyImageView) {
			...
			float min_distance = ScreenUtils.getDeviceWidth(context) / 4f;
			...
			if ((int) width <= ScreenUtils.getDeviceWidth(context)
					&& (int) height <= ScreenUtils.getDeviceHeight(context))// 如果图片当前大小<屏幕大小，直接处理滑屏事件
			{
				...
				if (distanceX > 0)// 向左滑动
				{
					if (r.left > 0 || right < ScreenUtils.getDeviceWidth(context)) {// 判断当前ImageView是否显示完全
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
				...
				{
					if (r.right < ScreenUtils.getDeviceWidth(context) || left > 0) {
						super.onScroll(e1, e2, distanceX, distanceY);
					} else {
						imageView.postTranslate(-distanceX, -distanceY);
					}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (view instanceof MyImageView) {
				...
				// + height + ",screenWidth="
				// + MyScreenUtils.getDeviceWidth(context) + ",screenHeight="
				// + MyScreenUtils.getDeviceHeight(context));
				if ((int) width <= ScreenUtils.getDeviceWidth(context)
						&& (int) height <= ScreenUtils.getDeviceHeight(context))// 如果图片当前大小<屏幕大小，判断边界
				{
					break;
				}
				float v[] = new float[9];
				...
				if (top < 0 && bottom < ScreenUtils.getDeviceHeight(context)) {
					// imageView.postTranslateDur(-top, 200f);
					imageView.postTranslateDur(
							ScreenUtils.getDeviceHeight(context) - bottom, 200f);
				}
				if (top > 0 && bottom > ScreenUtils.getDeviceHeight(context)) {
					// imageView.postTranslateDur(ScreenUtils.getDeviceHeight(context)
					// - bottom, 200f);
					imageView.postTranslateDur(-top, 200f);
				}
				...
				if (left < 0 && right < ScreenUtils.getDeviceWidth(context)) {
					// imageView.postTranslateXDur(-left, 200f);
					imageView.postTranslateXDur(
							ScreenUtils.getDeviceWidth(context) - right, 200f);
				}
				if (left > 0 && right > ScreenUtils.getDeviceWidth(context)) {
					// imageView.postTranslateXDur(ScreenUtils.getDeviceWidth(context)
					// - right, 200f);
					imageView.postTranslateXDur(-left, 200f);
				}
			}
